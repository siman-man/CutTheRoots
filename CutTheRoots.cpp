...
class CutTheRoots {
  public:
    ...
    void searchRoot(int rootId) {
      Vector *root = getVertex(rootId);
      int nsize = root->neighbor.size();

      //fprintf(stderr,"root.y = %d, root.x = %d, nsize = %d\n", (int)root->y, (int)root->x, nsize);
